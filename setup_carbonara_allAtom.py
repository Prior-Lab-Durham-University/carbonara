...
def write_runme(
    ...
    with open(run_file, "w+") as fout:
        ...
        fout.write("set +m   # ensure background jobs stay in same job-control context\n\n")
        ...
        fout.write("# ========= NEW: bookkeeping for clean shutdown =========\n")
        fout.write("PIDS=()\n")
        fout.write('WATCHER_PID=""\n')
        fout.write("cleanup() {\n")
        fout.write('    echo\n')
        fout.write('    echo ">>> Stopping Carbonara frontend..."\n\n')
        fout.write("    # Stop watcher first\n")
        fout.write('    if [[ -n "${WATCHER_PID}" ]]; then\n')
        fout.write('        kill -INT "$WATCHER_PID" 2>/dev/null || true\n')
        fout.write("        sleep 0.5\n")
        fout.write('        kill -TERM "$WATCHER_PID" 2>/dev/null || true\n')
        fout.write("        sleep 0.5\n")
        fout.write('        kill -KILL "$WATCHER_PID" 2>/dev/null || true\n')
        fout.write("    fi\n\n")
        fout.write("    # Stop all predictor processes explicitly\n")
        fout.write('    if ((${#PIDS[@]})); then\n')
        fout.write('        echo ">>> Stopping predictor processes (${#PIDS[@]})"\n')
        fout.write('        kill -INT  "${PIDS[@]}" 2>/dev/null || true\n')
        fout.write("        sleep 1\n")
        fout.write('        kill -TERM "${PIDS[@]}" 2>/dev/null || true\n')
        fout.write("        sleep 1\n")
        fout.write('        kill -KILL "${PIDS[@]}" 2>/dev/null || true\n')
        fout.write("    fi\n\n")
        fout.write('    echo ">>> Stopped."\n')
        fout.write("    exit 0\n")
        fout.write("}\n")
        fout.write("trap cleanup SIGINT SIGTERM\n")
        fout.write("# ======================================================\n\n")
        ...
        fout.write("# ========= NEW: watcher / backmapping / FoXS =========\n")
        fout.write('WATCHER_SCRIPT="$ROOT/watch_and_backmap.py"\n')
        fout.write('BACKMAP_SCRIPT="$ROOT/backmap_cli.py"\n')
        fout.write('WATCHER_LOG="$predictionFile/watcher.out"\n')
        fout.write('FOXS_PY="$ROOT/external/pyFoXS/pyFoXS/foxs.py"\n\n')

        fout.write('if [[ ! -f "$WATCHER_SCRIPT" ]]; then\n')
        fout.write('    echo "ERROR: watcher script not found: $WATCHER_SCRIPT"\n')
        fout.write("    exit 1\n")
        fout.write("fi\n")
        fout.write('if [[ ! -f "$BACKMAP_SCRIPT" ]]; then\n')
        fout.write('    echo "ERROR: backmap script not found: $BACKMAP_SCRIPT"\n')
        fout.write("    exit 1\n")
        fout.write("fi\n")
        if do_foxs:
            fout.write('if [[ ! -f "$FOXS_PY" ]]; then\n')
            fout.write('    echo "ERROR: Required FoXS script not found:"\n')
            fout.write('    echo "       $FOXS_PY"\n')
            fout.write('    echo "Run ./setup.sh first."\n')
            fout.write("    exit 1\n")
            fout.write("fi\n")
        fout.write("\n")

        fout.write('python "$WATCHER_SCRIPT" \\\n')
        fout.write('    --watch-dir "$predictionFile" \\\n')
        fout.write(f'    --scenario-root "$ROOT/{data_path}" \\\n')
        fout.write('    --backmap-script "$BACKMAP_SCRIPT" \\\n')
        fout.write(f"    --max-backmap {int(max_backmap)} \\\n")
        fout.write(f"    --defer-backmap-seconds {int(defer_backmap_seconds)}")
        if do_foxs:
            fout.write(' \\\n')
            fout.write('    --do-foxs \\\n')
            fout.write('    --foxs-py "$FOXS_PY" \\\n')
            fout.write('    --saxs "$ScatterFile" \\\n')
            fout.write('    --max-q "$kmax"')
        fout.write(' \\\n')
        fout.write('    > "$WATCHER_LOG" 2>&1 &\n')
        fout.write('WATCHER_PID=$!\n')
        fout.write('echo "Watcher started (PID=$WATCHER_PID)"\n')
        fout.write("# ==================================================\n\n")
        ...
        fout.write("    # NEW: stdbuf ensures live output, PID captured\n")
        fout.write('    stdbuf -oL -eL \\\n')
        fout.write('    $ROOT/build/bin/predictStructureQvary \\\n')
        fout.write('        $ScatterFile \\\n')
        fout.write('        $fileLocs \\\n')
        fout.write('        $initialCoordsFile \\\n')
        fout.write('        $pairedPredictions \\\n')
        fout.write('        $fixedsections \\\n')
        fout.write('        $noStructures \\\n')
        fout.write('        $withinMonomerHydroCover \\\n')
        fout.write('        $kmin \\\n')
        fout.write('        $kmax \\\n')
        fout.write('        $kmaxStart \\\n')
        fout.write('        $maxNoFitSteps \\\n')
        fout.write('        $predictionFile/mol$i \\\n')
        fout.write('        $scatterOut/scatter$i.dat \\\n')
        fout.write('        $mixtureFile \\\n')
        fout.write('        $prevFitStr \\\n')
        fout.write('        $logLoc/fitLog$i.dat \\\n')
        fout.write('        $endLinePrevLog \\\n')
        fout.write('        $affineTrans \\\n')
        fout.write('        $useErrors \\\n')
        fout.write('        > "$predictionFile/run$i.out" 2> "$predictionFile/run$i.err" &\n\n')

        fout.write('    PIDS+=($!)\n')
        fout.write("done\n\n")

        fout.write('echo\n')
        fout.write('echo ">>> All runs launched"\n')
        fout.write('echo ">>> Press Ctrl+C to stop everything"\n')
        fout.write('echo\n\n')
