...
def update_summary_file(summary_file: Path, aa_pdb: Path, value: str):
    summary_file.parent.mkdir(parents=True, exist_ok=True)
    existing_lines = []
    if summary_file.exists():
        existing_lines = summary_file.read_text().splitlines()

    aa_pdb_str = str(aa_pdb)
    filtered = [line for line in existing_lines if not line.startswith(aa_pdb_str + " ")]
    filtered.append(f"{aa_pdb_str} {value}")
...
def run_foxs(pyfoxs_cmd: str, aa_pdb: Path, saxs_dat: Path, max_q: float,
    ...
    if summary_file is not None:
        if p.returncode == 0 and chisq is not None:
            update_summary_file(summary_file, aa_pdb, chisq)
        else:
            update_summary_file(summary_file, aa_pdb, "ERROR")
